...
void main(void)
{
    ...
    ADMUX = 0x01;
    // AD Converter 7bit: 동작 허용, 6bit: 변환X, 5bit: free running mode, 3bit: 인터럽트 비허용, 2~0bit: 분주비 128
    ADCSRA = 0xe7;
    delay_ms(5);

    while (1)
    {
        // AD Converter 7bit: 동작 허용, 6bit: 변환O, 5bit: free running mode, 3bit: 인터럽트 비허용, 2~0bit: 분주비 128
        ADCSRA = 0xc7;
        // 4bit: 컨버전이 끝나면 1
